...
4. ### Read the rejection
   ...
   What the plan contains:
     Write(file:/.../demo/output/result_broken_example.parquet, format=parquet, saveMode=overwrite)
     └─ Project
        ├─ Project
...
<Aside type="note" title="Same install mechanism as your own job would use">
`DemoJobHarness` installs Invaract the same way [Install the Enforcement
Rule](/guides/installing-the-enforcement-rule/) recommends for a real job — purely via
`spark.sql.extensions` and `spark.invaract.contract`, no `ContractEnforcementRule` call of
its own. The command above doesn't show those as `--conf` flags because `DemoJobHarness`
sets them itself, from the contract path you already passed as its fourth argument.
</Aside>
