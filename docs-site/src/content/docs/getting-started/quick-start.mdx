...
1. ### Run the test harness
   ...
   2. Runs a real Spark job — `DemoJobHarness` — via actual `spark-submit`, with
      Invaract's `ContractEnforcementRule` installed in the `SparkSession`.
   3. The job reads `demo/input/sample.csv`, runs a small example transformation
      (`InvaractPlugin`, which adds two computed columns — `value_squared` and
      `value_tier`), and writes the result to `demo/output/result.parquet` — a write
      that Invaract verifies against `demo/contracts/invaract_output.yaml` before it's
      allowed to happen.
...
2. ### View the result
   ...
   - The translated **Transformation IR** — the actual Spark plan Invaract checked
   - **Contract verification**: `PASSED (invaract_demo_output@1.0.0)`, no violations

3. ### See it actually block a bad write

   Everything so far proves a *passing* write goes through. To see Invaract reject a
   violating one:
...
- [Infer a Starting Contract with Dry-Run Mode](/guides/dry-run-mode/) — run
  `./dev/dry-run` to get a contract printed from a real run, instead of writing one
  from a blank file.
...
- [Install the Enforcement Rule](/guides/installing-the-enforcement-rule/) — wire
  Invaract into your own Spark job, not just the demo.
