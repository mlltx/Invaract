...
1. ### Run the test harness
   ...
   3. The job reads `demo/input/sample.csv`, runs a small example transformation
      (`InvaractPlugin`, which adds a `value_squared` column), and writes the result to
      `demo/output/result.parquet` — a write that Invaract verifies against
      `demo/contracts/invaract_output.yaml` before it's allowed to happen.
