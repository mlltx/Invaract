...
description: What an Invaract contract is, and its relationship to ODCS.
sidebar:
  order: 1
---

A data contract, in Invaract, is a plain YAML document describing what a transformation
...
Invaract doesn't invent a new contract syntax. The shape mirrors the
...
1. **Unrecognized top-level keys are preserved, not rejected.** A contract authored with
   additional ODCS fields Invaract doesn't yet interpret (owners, SLAs, quality rules,
   and so on) still parses successfully — those keys land in the contract's `extensions`
   verbatim. Invaract is additive to the ecosystem, not a competing, incompatible
   format.
2. **Only the fields needed for verification are strongly typed.** `id`, `version`,
   ...
   Invaract carries but does not act on.
...
[What is Invaract?](/introduction/what-is-this/) for how the pieces fit
together end to end.
