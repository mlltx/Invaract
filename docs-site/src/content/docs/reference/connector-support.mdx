...
|---|---|---|---|---|---|
| **Delta Lake** | ✅ | ✅ | ✅ | 3.3.3 only | Row-level DML translated via a dedicated extractor; schema-evolution and generated-column edge cases investigated and fixed. Earlier releases (including 3.3.0–3.3.2) target an older Spark version this repo has already moved past — 3.3.3 is the only compatible release, not just the newest. |
| **Iceberg** | ✅ | ✅ | ✅ (copy-on-write fully; merge-on-read `UPDATE` fails closed) | 1.10.2 – 1.11.0 | Catalog-based DSv2 connector. `CALL` procedures (e.g. `rollback_to_snapshot`) are classified individually — some verified against a contract, others safely allowed through, per procedure. |
