...
**Contributor-facing project docs (`ROADMAP.md`, `ARCHITECTURE.md`, `docs/**`) stay
plain Markdown at those paths — they are the source of truth, and `docs-site/` MUST
NOT mirror them** (see `docs-site/DOCUMENTATION.md`'s "What belongs in user
documentation vs. elsewhere"). `contributor-docs/` is a separate, second Astro
Starlight site — same reading experience, different audience and different content —
that gives contributors a browsable/searchable rendering of exactly those files
without blurring that line: every page under `contributor-docs/src/content/docs/`
except `index.mdx` is generated by `contributor-docs/scripts/sync-docs.mjs` straight
from `ROADMAP.md`/`ARCHITECTURE.md`/`docs/**`, so there is still only one place each
piece of contributor content is actually written. Never hand-edit a generated
`contributor-docs/` page or add net-new prose there — edit the real source file, then
run `npm run sync` (or just `npm run dev`/`build`, which do it automatically). See
...
│   └── astro.config.mjs
│
├── contributor-docs/              # Contributor documentation: a SEPARATE Astro Starlight
│   │                               # site, deployed to /contributor/ on the same GitHub Pages
│   │                               # deployment. Same reading experience as docs-site/, but for
│   │                               # people working on Invaract's own codebase — never shown to
│   │                               # or aimed at a user of the verification engine. Every page
│   │                               # under src/content/docs/ except index.mdx is GENERATED
│   │                               # (gitignored) by scripts/sync-docs.mjs, straight from
│   │                               # ROADMAP.md/ARCHITECTURE.md/docs/** — there is no content to
│   │                               # hand-edit here; see contributor-docs/README.md.
│   ├── scripts/sync-docs.mjs     # Source of truth for what's mirrored and how links rewrite
│   ├── src/content/docs/index.mdx # The one hand-authored page (landing page)
│   └── astro.config.mjs
│
├── .github/workflows/
...
│   └── deploy-contributor-docs.yml # Builds and deploys contributor-docs/ to the same
│                                    # deployment's /contributor/ subfolder
│
...
- `contract/target/scala-2.12/invaract-contract-0.10.0.jar`
- `ir/target/scala-2.12/invaract-ir-0.5.0.jar`
- `fingerprint/target/scala-2.12/invaract-fingerprint-0.3.0.jar`
- `spark-adapter/target/scala-2.12/invaract-spark-adapter-0.8.0.jar` — via
...
spark-shell --jars plugin/target/scala-2.12/invaract-spark-plugin-0.2.0.jar,spark-adapter/target/scala-2.12/invaract-spark-adapter-0.8.0.jar
