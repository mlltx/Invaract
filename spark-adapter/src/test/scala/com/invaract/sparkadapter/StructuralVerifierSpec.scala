...
class StructuralVerifierSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("golden example: UNDECLARED_OUTPUT_COLUMN, matching the spec's exact violation shape") {
    ...
    val plan = com.invaract.ir.Write(
      ...
      com.invaract.ir.Project(
        ...
        List(
          com.invaract.ir.NamedExpr("customer_id", com.invaract.ir.ColumnReference(com.invaract.ir.ColumnRef("customer_id"))),
          com.invaract.ir.NamedExpr("country", com.invaract.ir.ColumnReference(com.invaract.ir.ColumnRef("country")))
        )
      )
    )
  ...
  test("location matching bridges a contract's relative path and Spark's absolute file: URI") {
    ...
    val reads = {
      def collect(p: com.invaract.ir.Plan): List[Read] = p match {
        case r: Read => List(r)
