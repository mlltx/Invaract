...
// bump requires.
// 0.4.0 -> 0.5.0: notification.WriteEvent gained a trailing
// `partitionColumns: List[String]` constructor parameter (a write's
// published message now reports which column(s)/transform(s) its target
// is partitioned by, when knowable - see WriteCommandInfo.partitionColumns'
...
libraryDependencies ++= Seq(
  ...
  "com.invaract" %% "invaract-contract" % "0.10.0",
...
//
// The 0.3.0 -> 0.4.0 bump landed on the base branch in its own PR, which
// left this pointing at the now-superseded 0.3.0 baseline with a
// "FOLLOW-UP: flip this once that PR lands" comment - the same pattern
// contract/ir's own 0.2.0 -> 0.3.0 bumps already went through (see
// ir/build.sbt's matching comment). That PR has now landed (base-ref
...
// so this file's mimaPreviousArtifacts was flipped to 0.4.0 - the follow-up
// flip for that bump, done in a separate PR, per this section's own
