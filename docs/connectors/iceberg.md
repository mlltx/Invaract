...
| Contract DML rule verification (`merge_condition`/`forbid_unconditional_delete`/`allowed_update_columns` — a separate feature from the structural DML row above, see docs/SPARK_ADAPTER.md's "DML rule verification") | 🔧 **Found and (mostly) fixed** | `merge_condition`/`forbid_unconditional_delete` work identically to Delta's for both write strategies — `RowLevelWrite.condition`/`.operation.command()` are real public fields, no reflection needed. `allowed_update_columns` works for copy-on-write's `ReplaceData` (its rewritten `Project` wraps every column in `If(cond, assignedExpr, originalAttr)`, letting a changed column be detected) but is genuinely unextractable for merge-on-read's `WriteDelta` (an `Expand`-based rewrite with no per-column pairing) — this fails closed (`RULE_UNVERIFIABLE_DML`) rather than silently passing. `IcebergConnectorSpec`'s PASS/FAIL pairs per rule type, plus the merge-on-read `RULE_UNVERIFIABLE_DML` case and its no-rule-declared counterpart. |
