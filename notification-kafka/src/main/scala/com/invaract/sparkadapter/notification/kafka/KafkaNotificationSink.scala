...
import com.invaract.sparkadapter.notification.{NotificationEvent, NotificationJson, NotificationSink}
...
/** Publishes every event, as JSON keyed by its `eventType`, to a Kafka
  ...
  * `sink.class` at `com.invaract.sparkadapter.notification.kafka.KafkaNotificationSink`
